...
public class FoodController {
	...
	@GetMapping("food/food_before_list_detail.do") // 쿠키
	public String food_before_detail(int fno, RedirectAttributes ra, HttpServletResponse response)
	{
		Cookie cookie=new Cookie("food_" + fno, String.valueOf(fno)); // cookie는 문자열만 저장이 가능하기 때문에 int인 fno를 String으로 바꿔줘야 함
		// Cookie(String, String)
		cookie.setPath("/");
		cookie.setMaxAge(60*60*24);
		response.addCookie(cookie);
		ra.addAttribute("fno", fno);
		return "redirect:../food/food_list_detail.do"; // 쿠키가 저장된 후 디테일로 이동
	}
